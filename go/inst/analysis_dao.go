...
func GetReplicationAnalysis(clusterName string, hints *ReplicationAnalysisHints) ([]ReplicationAnalysis, error) {
		...
		analysisQueryReductionClause = `
			HAVING
				...
				OR (IFNULL(SUM(subordinate_instance.last_checked <= subordinate_instance.last_seen
		                    ...
		                    AND subordinate_instance.last_io_error like '%error %connecting to main%'
	...
	query := fmt.Sprintf(`
		    SELECT
		        ...
		        main_instance.port,
						main_instance.data_center,
						main_instance.physical_environment,
		        ...
		        GROUP_CONCAT(concat(subordinate_instance.Hostname, ':', subordinate_instance.Port)) as subordinate_hosts,
		        MIN(
		            main_instance.subordinate_sql_running = 1
		            AND main_instance.subordinate_io_running = 0
		            AND main_instance.last_io_error like '%%error %%connecting to main%%'
              ...
              0) AS count_row_based_loggin_subordinates,
						IFNULL(MIN(subordinate_instance.gtid_mode), '')
              AS min_replica_gtid_mode,
						IFNULL(MAX(subordinate_instance.gtid_mode), '')
              ...
              0) AS count_downtimed_replicas,
						COUNT(DISTINCT case
								when subordinate_instance.log_bin AND subordinate_instance.log_subordinate_updates
								then subordinate_instance.major_version
								else NULL
							end
          ...
          LEFT JOIN
		        database_instance_downtime as main_downtime ON (main_instance.hostname = main_downtime.hostname
					...
					LEFT JOIN
		        database_instance_downtime as replica_downtime ON (subordinate_instance.hostname = replica_downtime.hostname
		        		AND subordinate_instance.port = replica_downtime.port
	...
	err := db.QueryOrchestrator(query, args, func(m sqlutils.RowMap) error {
		...
		a.CountReplicas = m.GetUint("count_subordinates")
		...
		a.CountReplicasFailingToConnectToMain = m.GetUint("count_subordinates_failing_to_connect_to_main")
		...
		if a.IsMain && !a.LastCheckValid && a.CountReplicas == 0 {
			...
			//
		} else if a.IsMain && !a.LastCheckValid && a.CountStaleReplicas == a.CountReplicas && a.CountValidReplicatingReplicas > 0 {
			a.Analysis = UnreachableMainWithStaleSubordinates
			a.Description = "Main cannot be reached by orchestrator and has running yet stale replicas"
			//
			...
			a.Description = "Main is reachable but none of its replicas is replicating"
			//
		} else if a.IsMain && a.LastCheckValid && a.CountReplicas > 1 && a.CountStaleReplicas == a.CountReplicas && a.CountValidReplicas > 0 && a.CountValidReplicatingReplicas > 0 {
			a.Analysis = AllMainSubordinatesStale
			a.Description = "Main is reachable but all of its replicas are stale, although attempting to replicate"
			//
