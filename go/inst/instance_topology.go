...
// DetachReplicaOperation will detach a replica from its main by forcibly corrupting its replication coordinates
...
// ReattachReplicaOperation will detach a replica from its main by forcibly corrupting its replication coordinates
...
// ResetMainGTIDOperation will issue a safe RESET MASTER on a replica that replicates via GTID:
...
func ResetMainGTIDOperation(instanceKey *InstanceKey, removeSelfUUID bool, uuidToRemove string) (*Instance, error) {
	...
	if len(instance.SubordinateHosts) > 0 {
		return instance, log.Errorf("reset-main-gtid will not operate on %+v because it has %+v replicas. Expecting no replicas", *instanceKey, len(instance.SubordinateHosts))
	}
...
func TakeMain(instanceKey *InstanceKey) (*Instance, error) {
	instance, err := ReadTopologyInstance(instanceKey)
	if err != nil {
		return instance, err
	}
